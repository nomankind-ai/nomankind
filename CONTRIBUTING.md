...
- **Entries.** Facts are submitted as signed entries that conform to [`schema/nomankind-entry-schema.json`](schema/nomankind-entry-schema.json). An entry states what a cited primary source said or what a reproducible transcript shows — never a characterization or an opinion. Each entry carries a citation and a snapshot hash computed by the [norm-v1 rule](schema/nomankind-snapshot-normalization-v1.md).
- **Disputes and corrections.** A challenge is itself an entry in the `correction` category, with its own citation. Evidence wins; unfounded challenges cost standing.

## Ground rules

- One atomic, cited claim per entry. If you can't cite it, it doesn't belong.
...
- Be neutral in tone. The log records what a source said, and lets readers decide what it means.
