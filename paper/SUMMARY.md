...
A summary of the whitepaper (v1.7). The full paper is [WHITEPAPER.md](WHITEPAPER.md).
...
Stated entries are about the source, not the world. Observer frames (region, account tier) are not yet recorded. Measurement is unfunded and always will be: whoever measures pays for the call. The confidence field is null until there is dispute history to calibrate it. The identity layer, 1F916, is a v0.0 draft. Genesis is the weak spot: below ten trusted operators the random draw defends nothing, and the first pool is the maintainer's own, disclosed as one perimeter on every entry it decides until outside operators clear it. Nobody mandates adoption. The paper lists each of these with what it binds.
