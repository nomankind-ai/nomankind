...
describe("one cache over many entries", () => {
  it("reads the registry once, and gathers the same worlds", async () => {
    ...
    // Ten registry event types, paged to exhaustion: ten statements for the
    // one reading, and ten per entry without the cache. Eight since D-138 put
    // the two community operator events in the registry set — one registry,
    // two kinds of operator, and a world that skipped either would tell every
    // door no community operator had ever registered — nine since D-095
    // added `key_rotated`, without which a world would count a retired key's
    // decisions and refuse the key that replaced it, and ten since D-142 added
    // `community_operator_bound`, without which a world would judge a key-bound
    // operator by the account it registered as.
    expect(without).toBe(10 * ENTRIES);
  ...
  it("shares the one reading between gatherings made at once", async () => {
    ...
    expect(counting.count(REGISTRY_QUERY)).toBe(10);
