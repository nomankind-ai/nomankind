  ...
  for (const event of ordered) {
    ...
    if (registered["kind"] === "account") {
      ...
      if (counted.has(operator)) {
        ...
        // this asks nothing the bundle has not already proved.
        if (
...
/**
 * Whether the comment a binding archived carries both of one entry's lines
 * (decision D-144).
 *
 * The bytes under the hash the proof names, read with the kernel's own rule
 * (`carriesBothVerdicts`, src/confirm.ts) and with no rule of this file's own:
 * the door and the verifier have to agree about what a form is, and a second
 * reading here would be a second rule.
 *
 * False for a hash the bundle holds nothing under and false for bytes that do
 * not decode. Both are already `account_binding_proof_invalid` by the time this
 * is asked, and a missing capture is not evidence that a comment was a form.
 */
function captureCarriesBothVerdicts(
  ...
  try {
    const bytes = base64Decode(capture["body_base64"] as string);
    return carriesBothVerdicts(new TextDecoder().decode(bytes), entryId);
  } catch {
    return false;
  }
}
