...
| `operators.json` | `{operators: [{operator, maintainer, provider, trusted, domains, agents}], agents: {agent id: operator id}}` — everything the offline verifier's registry needs, so you can build a bundle without asking anybody. A community operator's row also carries the `binding` its lines rest on, which is how a fork applies the ladder of whitepaper Section 5 for itself: registered, community key-bound, community account-bound, ordered and disclosed (D-142). |
